define([
	...
	'./util',
	'./strategies'
], function (lang, whenAll, Command, util, strategies) {
	/**
				...
				.then(function (element) {
					return this.executeAsync(/* istanbul ignore next */ function (element, timeout, done) {
						...
						(function poll() {
							...
							else if (+new Date() - startDate > timeout) {
								done(false);
							}
							else {
								setTimeout(poll, 200);
					...
					}, [ element, timeout - (startDate - Date.now()) ]);
	...
	var methods = {
		...
		newWindow: function (url, name) {
			...
			return this.execute(/* istanbul ignore next */ function (url, name) {
		...
		getPageIndex: function (element) {
			...
			return new Command(this, function () {
				if (this.context.isSingle) {
					...
					return whenAll(this.context.map(function (element) {
						return element._get('pageIndex');
					}));
				}
			});
