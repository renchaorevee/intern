define([
	...
	'./strategies',
	'./util'
], function (lang, Deferred, Element, statusCodes, storage, strategies, util) {
	...
	Session.prototype = {
		...
		switchToWindow: function (name) {
			...
			return this._post('window', {
				...
				// ios-driver 0.6.6-SNAPSHOT April 2014 fails to handle the case where a window name is provided
				if (self.capabilities.browserName === 'Safari' && self.capabilities.platformName === 'IOS' &&
					error.message.indexOf('No enum constant org.uiautomation.ios.UIAModels.configuration.WorkingMode') > -1
				) {
					// Always sleep before attempting to select a window because ios-driver 0.6.6-SNAPSHOT April 2014
					// does not wait until iOS animation for opening a new window completes before attmepting to
					// access the new window
					return util.sleep(3000).then(function () {
						return self.execute(function (name) {
		...
		setCookie: function (cookie) {
			...
			return this._post('cookie', {
				...
				if (error.name === 'UnknownCommand') {
					var cookieToSet = [ encodeURIComponent(cookie.name) + '=' + encodeURIComponent(cookie.value) ];
		...
		getAvailableLogTypes: function () {
			var self = this;
			return this._get('log/types').otherwise(function (error) {
				...
				if (self.capabilities.browserName === 'selendroid' && !error.response.text.length) {
					return [ 'logcat' ];
				}
