define([
	...
	'./Element',
	'./util'
], function (Deferred, whenAll, Session, Element, util) {
	/**
	...
	function createElementMethod(method) {
		return function () {
			...
			return new Command(this._session, this, function () {
	...
	function Command(session, parent, initialiser) {
		...
		if (initialiser) {
			...
			this._lastPromise = promise.then(function (newContext) {
				context.push.apply(context, Array.isArray(newContext) ? newContext : [ newContext ]);
			});
		}
		else {
