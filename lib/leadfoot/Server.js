		...
		return function (path, requestData, pathParts) {
			...
			return request(url, kwArgs).response.then(function handleResponse(response) {
				...
				if (response.status === 204) {
					...
					// At least InternetExplorerDriver 2.41.0 responds with HTTP status codes other than Not Implemented
					// and a Selenium status UnknownError for commands that are not implemented; like FirefoxDriver
					// it provides a reliable indicator of unsupported commands
					if (response.status === 500 && data.value && data.value.message.indexOf('Command not found') > -1) {
						data.status = 9;
					}
		...
		_fixSessionCapabilities: function (session) {
			...
			function discoverDefects() {
				...
				testedCapabilities.brokenExecuteElementReturn = function () {
					return get('<!DOCTYPE html><style>html{background-color:blue;}</style><div id="a"></div>').then(function () {
				...
				testedCapabilities.brokenElementDisplayedOpacity = function () {
					return get('<!DOCTYPE html><style>html{background-color:green;}</style><div id="a" style="opacity: 0;">a</div>').then(function () {
				...
				if (capabilities.mouseEnabled) {
					...
					testedCapabilities.brokenDoubleClick = function () {
						/*jshint maxlen:200 */
						return get('<!DOCTYPE html><style>html{background-color:yellow;}</style><script>counter = 0; var d = document; d.onclick = d.onmousedown = d.onmouseup = function () { counter++; };</script>').then(function () {
				...
				if (capabilities.touchEnabled) {
					...
					var scrollTestUrl = '<!DOCTYPE html><style>html{background-color:orange;}</style><div id="a" style="margin: 3000px;"></div>';
