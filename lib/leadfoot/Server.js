...
define([
	...
	'./Session',
	'./util'
], function (Deferred, lang, request, whenAll, statusCodes, keys, Session, util) {
	/**
	 ...
	 * JsonWireProtocol HTTP server is [http://localhost:4444/wd/hub].
	 */
	function Server(url) {
	...
	Server.prototype = /** @lends module:leadfoot/Server# */ {
		 ...
		 *
		 * @returns {Promise.<Object>} An object containing arbitrary properties describing the
		 */
		getStatus: function () {
		...
		createSession: function (desiredCapabilities, requiredCapabilities) {
			...
			return this._post('session', {
				...
				return self._fixSessionCapabilities(session);
			});
		},
		...
		/**
		 * Gets information on the capabilities of a given session.
		 *
