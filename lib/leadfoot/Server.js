define([
	...
	Server.prototype = {
		...
		_fixSessionCapabilities: function (session) {
			...
			// selendroid 0.9.0 has broken cookie deletion
			...
			// At least ios-driver 0.6.6-SNAPSHOT does not support execute_async
			...
			return whenAll(testedCapabilities).then(function (testedCapabilities) {
				...
				if (capabilities.touchEnabled) {
					...
					var windowTestUrl = 'data:text/html;charset=utf-8,' +
						encodeURIComponent('<!DOCTYPE html><a target="foo" href="about:blank">a</a>');
					...
					return session.get(scrollTestUrl).then(function () {
						...
						capabilities.brokenTouchScroll = error.message;
					})

					// ios-driver 0.6.6-SNAPSHOT April 2014 fails to discard closed window handles
/* TODO: ios-driver corrupts its internal state when windows are opened and closed so we need to not test this or else
   the entire test run will lock up
					.then(function () {
						return session.get(windowTestUrl);
					}).then(function () {
						return session.getAllWindowHandles();
					}).then(function (handles) {
						return session.getElementByTagName('a').then(function (element) {
							return element.click();
						}).then(function () {
							return util.sleep(3000);
						}).then(function () {
							return session.execute(function () {
								window.open('', 'foo').close();
							});
						}).then(function () {
							return session.getAllWindowHandles();
						}).then(function (newHandles) {
							capabilities.brokenDeleteWindow = newHandles.length > handles.length;
							return session.switchToWindow(handles[0]);
						});
					}).otherwise(function (error) {
						capabilities.brokenDeleteWindow = error.message;
					});*/
				}
			}).always(function () {
